...
class ChrominiumHelper {
    ...
    static getLaunchOptions(capabilities) {
        ...
        const windowPosition = capabilities.get(CapabilityDefinitions.SupportedCapabilities.WINDOW_POSITION)
        if (windowPosition !== undefined && /\d+,\d+/.test(windowPosition)) {
            launchOptions.push(`--window-position=${windowPosition}`)
        }

        const windowSize = capabilities.get(CapabilityDefinitions.SupportedCapabilities.WINDOW_SIZE)
        if (windowSize !== undefined && /\d+,\d+/.test(windowSize)) {
            launchOptions.push(`--window-size=${windowSize}`)
        }

        const allowFileAccessFromFiles = capabilities.get(CapabilityDefinitions.SupportedCapabilities.ALLOW_FILE_ACCESS_FROM_FILES)
        if (allowFileAccessFromFiles !== undefined && allowFileAccessFromFiles) {
            launchOptions.push(`--allow-file-access-from-files`)
        }

        const useFakeDeviceForMediaStream = capabilities.get(CapabilityDefinitions.SupportedCapabilities.USE_FAKE_DEVICE_FOR_MEDIA_STREAM)
        if (useFakeDeviceForMediaStream !== undefined && useFakeDeviceForMediaStream) {
            launchOptions.push(`--use-fake-device-for-media-stream`)

        }

        const useFakeUIForMediaStream = capabilities.get(CapabilityDefinitions.SupportedCapabilities.USE_FAKE_UI_FOR_MEDIA_STREAM)
        if (useFakeUIForMediaStream !== undefined && useFakeUIForMediaStream) {
            launchOptions.push(`--use-fake-ui-for-media-stream`)
        }

        const disableGPU = capabilities.get(CapabilityDefinitions.SupportedCapabilities.DISABLE_GPU)
        if (disableGPU !== undefined && disableGPU) {
            launchOptions.push(`--disable-gpu`)
        }

        const disableInfobars = capabilities.get(CapabilityDefinitions.SupportedCapabilities.DISABLE_INFOBARS)
